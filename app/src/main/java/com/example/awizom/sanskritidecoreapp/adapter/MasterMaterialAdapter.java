...
public class MasterMaterialAdapter extends RecyclerView.Adapter<MasterMaterialAdapter.OrderItemViewHolder> {
    ...
    public MasterMaterialAdapter(Context mCtx, List<RoomModel> RoomModels) {
        this.mCtx = mCtx;
    ...
    @NonNull
    @Override
    public MasterMaterialAdapter.OrderItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.master_material_adapter, null);
        return new MasterMaterialAdapter.OrderItemViewHolder(view, mCtx, RoomModels);
    }

    @Override
    public void onBindViewHolder(@NonNull final MasterMaterialAdapter.OrderItemViewHolder holder, int position) {
        RoomModel RoomModel = RoomModels.get(position);
        try {

            holder.material_type.setText(RoomModel.getMaterialType());
            holder.materil_id.setText(String.valueOf(RoomModel.getMaterialID()));
                ...
                public void onClick(View v) {
                    ...
                    add.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            v.startAnimation(buttonClick);
                            try {
                                result = new OrderHelper.PostMaterialMaster().execute(holder.materil_id.getText().toString().trim(),addtext.getText().toString()).get();
                                if (result.isEmpty()) {
