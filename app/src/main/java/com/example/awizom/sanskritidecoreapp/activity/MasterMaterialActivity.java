...
import com.example.awizom.sanskritidecoreapp.adapter.MasterMaterialAdapter;
...
public class MasterMaterialActivity extends AppCompatActivity {

    RecyclerView recyclerView;
    private String result = "";
    List<RoomModel> RoomModels;
    MasterMaterialAdapter adapter;
    ImageButton addRoom;
    ...
    private void initView() {
        ...
        toolbar.setTitle("Material List");
    ...
    private void getRoomList() {
        ...
        try {
            ...
            result = new OrderHelper.GetMaterialDetail().execute(SharedPrefManager.getInstance(this).getUser().access_token).get();
            if (result.isEmpty()) {
                ...
                for (int i = 0; i < RoomModels.size(); i++) {
                    room_type.add(RoomModels.get(i).getMaterialType().toString());
                }
                Log.d("Error", RoomModels.toString());

                adapter = new MasterMaterialAdapter(this, RoomModels);
    ...
    private void postAddRoom() {
        ...
        try {
         ...
         result = new OrderHelper.PostMaterialMaster().execute(addtexttype.trim().toString()).get();
            ...
            try {
                if (result.isEmpty()) {
                    ...
                    Gson gson = new Gson();
                    Toast.makeText(this, "Material Successful", Toast.LENGTH_SHORT).show();
                    getRoomList();
